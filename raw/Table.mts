import type { keys, Holders, ANSIString } from "./types.mts"
import { MultiArray } from "./MultiArray.mts"
...
> {
		...
		try {
			...
			if (isMap(this.container)) {
				...
				if (holdsMap(this.container)) { // Map<Map>
					...
					this.container.get(row)![col as number] = value
				}
			} else {
				...
				if (holdsMap(this.container)) { // Array<Map>
					...
					else if (this.container[row as number].has(col))
						update = 2
					this.container[row as number].set(col, value)
				} else { // Array<Array>
					if (!rowExists)
						while (!(row in this.container))
							this.container.push(Array(this.numCols))
					else if (col in this.container[row as number])
						update = 2
					else while (!(col in this.container[row as number])) this.container[row as number].push(null as Cell)
					this.container[row as number][col as number] = value
				}
			}
			return update
		} catch (e) {
	...
	get(row: keys, col: keys): Cell | undefined {
		if (isMap(this.container)) {
			...
			return this.container.get(row)?.at(col as number)
		}
		if (holdsMap(this.container))
			return this.container.at(row as number)?.get(col)
		return this.container.at(row as number)?.at(col as number)
	}
		...
		if (isMap(this.container)) {
			if (holdsMap(this.container)) {
				...
				return Object.fromEntries(
					[...this.container as Map<keys, Map<keys, Cell>>].map(([key, map]) => [key,
						Object.fromEntries([...map])])
				) as unknown as Table<Cell, Form>["tableableData"]
			}
			this.container
			return Object.fromEntries([...this.container as Map<keys, Cell[]>]) as unknown as Table<Cell, Form>["tableableData"]
		}
		if (holdsMap(this.container))
